...
exports.batchGenerateDocs = batchGenerateDocs;
 ...
 */
async function batchGenerateDocs(documentText, languageId, onProgress) {
    ...
    for (let i = 0; i < undocumentedFunctions.length; i++) {
        ...
        try {
            ...
            const docsResult = await (0, docGenerator_1.generateDocs)(functionCode, 'senior');
 ...
 */
function parseFunctions(lines, languageId) {
    switch (languageId) {
        case 'python':
 ...
 */
function parsePythonFunctions(lines) {
    const functions = [];
 ...
 * Parses Java methods
 */
function parseJavaFunctions(lines) {
    const functions = [];
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
        const trimmed = line.trim();
